...
(function (Scratch) {
  ...
  canvasParent.addEventListener('mousedown', e => {
    //todo find a way to block the mouse when needed
    //e.stopImmediatePropagation();
  }, {capture: true});

  let clamp = (min, max, num) => { return (num <= min ? min : (num >= max ? max : num)) };
  ...
  class KeySimulation {
    getInfo() {
      return {
        ...
        blocks: [
          ...
          {
            opcode: "clickMouse",
            text: "click mouse button [BUTTON] at x: [X] y: [Y]",
            blockType: Scratch.BlockType.COMMAND,
            arguments: {
              BUTTON: {
                type: Scratch.ArgumentType.STRING,
          ...
          {
            opcode: "holdMouse",
            text: "click and hold mouse button [BUTTON] at x: [X] y: [Y] for [SECONDS] seconds",
            blockType: Scratch.BlockType.COMMAND,
            arguments: {
              BUTTON: {
                type: Scratch.ArgumentType.STRING,
    ...
    async clickMouse(args) {
      let BUTTON = clamp(0, 2, Scratch.Cast.toNumber(args.BUTTON));
      virtualX = Scratch.Cast.toNumber(args.X);
      virtualY = Scratch.Cast.toNumber(args.Y);
      //block mouse here
      postMouseData({
        isDown: true,
        button: BUTTON,
